...
Every application/service might receive encryption keys from OpenKCM Crypto for encrypting its own data at rest or
on necessity at transit.
...
OpenKCM Crypto is implementing a **four-level hierarchical keychain** (L1 → L4) using envelope encryption, where each parent key encrypts its child key:
...
2. **OpenKCM CMK & Crypto has capability to integrate with multiple keystores**:
...
4. **Key Strategy**:
    ...
    - L2: Tenant keys encrypted with L1. 
      Kept plaintext temporarily in memory for encrypt/decrypt L3 keys, is not persisted anywhere using plaintext payload.
    - L2.1: Same L2 tenant key encrypted with internal versioned rotated key (IVK) (can persisted on a storage).
    - L3: Service-level keys encrypted by L2
    - L4: ephemeral rotation per session/request encrypted by L3

5. Open to **support** BYOK, HYOK, CYOK, MYOK.
...
Encrypted Key material as example:
```text
---         |  -----
...
flowchart TD
    ...
    %% Definitions
    L1_plaintext([L1 Key<br/>🟩 Plaintext]):::green
    L2_ciphertext([L2 Key<br/>🟥 Wrapped by L1]):::red
    L2_plaintext([L2 Key<br/>🟩 Plaintext]):::green
    L3_ciphertext([L3 Key<br/>🟥 Wrapped by L2]):::red
    L3_plaintext([L3 Key<br/>🟩 Plaintext]):::green
    L4_plaintext([L4 DEK<br/>🟩 Plaintext]):::green
    L4_ciphertext([L4 DEK<br/>🟥 Wrapped by L3]):::red
...
#### OpenCMK Crypto Internal Versioned Keys (IVK)
- Root of L2 domain
- Encrypts L2 keys
...
- Encrypts L3 keys
- Wrapped by an internal versioned key (IVK)
...
2. *Scenario 2:* Generated and kept on OpenKCM Crypto Edge side
    ...
    - Retried from OpenKCM Crypto Edge by client to encrypt/decrypt actual data
    - Wrapped with L3 keys
