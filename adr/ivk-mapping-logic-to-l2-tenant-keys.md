...
* **High Latency:** Every time a Crypto Core node needs to unwrap an L2 (e.g., after a cache miss or restart), it must perform a cross-network call to the customer’s cloud KMS.
...
The Crypto Core will maintain a regional **IVK Registry**. Every L2 Tenant Key will transition from a single-wrapped state to a **Dual-Wrapped state**:
...
* **Scope:** One set of IVKs per Regional Crypto Core cluster.
...
When the Crypto Core receives a request for a Tenant's L2 key:
