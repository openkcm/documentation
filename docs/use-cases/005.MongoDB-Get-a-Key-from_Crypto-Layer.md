...
## Persona
**MongoDB Service** - A database service running within a Platform Mesh account that needs to encrypt/decrypt data using Krypton. MongoDB operates without customer involvement in key management and uses OpenKCM's internal key hierarchy for transparent encryption.

## Overview

As a MongoDB service running in Platform Mesh, I need to request encryption keys from Krypton to encrypt database records. I don't require customer-managed keys (CMK) — instead, I use OpenKCM's internal key management system for automatic, transparent encryption without customer key configuration.

## User Stories

### Story 1: MongoDB Registers with Krypton
**As a** MongoDB service instance  
**I want to** register with Krypton  
...
2. **Automatic Registration**: Platform Mesh automatically registers me with Krypton
...
3. **Certificate Provisioning**: I receive mTLS certificates from Platform Mesh
   - Client certificate for Krypton authentication
   - Certificate scoped to my tenant and service permissions
4. **Connection Establishment**: I establish persistent connection to Krypton
   - TLS 1.3 with mutual authentication
   - Connection pooling for performance
5. **Service Validation**: Krypton confirms my registration
...
2. **Key Request via KMIP**: I request encryption key from Krypton
   ```
   KMIP Request:
...
3. **Internal Key Derivation**: Krypton processes request
...
4. **Data Encryption**: I encrypt sensitive fields
   ```python
   # Get encryption key from Krypton
...
**As a** MongoDB service  
**I want to** decrypt stored data using keys from Krypton  
...
3. **Decryption Key Request**: I request decryption key via KMIP from Krypton
   ```
   KMIP Request:
...
4. **Data Decryption**: I decrypt the sensitive fields
   ```python
   # Get decryption key from Krypton
...
5. **Performance Optimization**: I cache keys briefly for performance
   - Keys cached for a short period to reduce round-trips
...
```
MongoDB Service → Krypton Authentication (mTLS)
        ↓
MongoDB → Request encryption key (KMIP)
        ↓
Krypton → Derive ephemeral key from internal L2 key
        ↓
...
- **REQ-001**: MongoDB must authenticate with Krypton using mTLS certificates
- **REQ-002**: Krypton must provide encryption keys using internal key hierarchy
...
- ✅ MongoDB service integrates transparently with Krypton
