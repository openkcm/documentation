# ACD-203: Crypto Gateway – High-Performance Ephemeral Data Plane
...
Operations that alter the fundamental state, policy, or lifecycle of a key require global consensus and audit trails. The Gateway acts as a secure, mTLS-authenticated proxy for these requests, tunneling them back to the **Crypto Core** which holds the authoritative registry.

## Architectural Components
The Crypto Gateway acts as a specialized, highly efficient forwarder with local caching capabilities.
...
3.  **L3 Wrapping:** The Crypto Gateway wraps the new L4 DEK using the locally cached **L3 Service Key** (which was previously unsealed by the Core).
...
2.  **Tunneling:** The Gateway opens a secure gRPC tunnel to the **OpenKCM Crypto Core**, forwarding the client's request context.
...
## Resilience & Autonomy
The Crypto Gateway is designed for **Partition Tolerance**. If the network link to the regional Crypto Core is severed:
...
## Summary
This document defines the **Crypto Gateway** as the high-speed tactical layer of OpenKCM. By restricting its autonomous scope to L4 `Create` and `Get` operations while providing secure local persistence, it ensures that:
