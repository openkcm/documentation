...
* **Purpose:** Acts as the **Key Encryption Key (KEK)** for high-volume operations. It is cached in memory by the Crypto Core (and pushed to the Crypto Gateway) to wrap and unwrap L4 keys.

### L4: The Data Encryption Key / DEK (Leaf)
* **Scope:** A single record, file, object, or session.
* **Owner:** The Application / Workload.
* **Storage:** Ephemeral. It is generated by the **Crypto Gateway**, used to encrypt the payload, and stored alongside the data (in the application database) encrypted (wrapped) by the L3 key.
...
3.  **L3 Distribution:** The plaintext L3 key is securely pushed (via mTLS) to the **Crypto Gateway's** secure memory cache.
4.  **L4 Execution:** The **Crypto Gateway** generates a plaintext L4 DEK for the application to use. It immediately encrypts a copy of this L4 DEK using the cached L3 key and returns the "Wrapped DEK" to the application for storage.
