...
The **Crypto** layer is the "Muscle." It consists of regional and edge nodes designed for zero-latency cryptographic operations.
...
* **OpenKCM Crypto Edge:** Distributed, high-speed interfaces that generate and manage **L4 (Data)** keys via KMIP. The Edge operates close to the workload, ensuring sub-millisecond encryption.
...
4.  **L4 (Data)**: Ephemeral keys used for per-record encryption, generated and stored at the Edge or client side.
...
### OpenKCM Crypto Edge
* **Dedicated Edge Service**: Deployed as a standalone service close to workloads, managing high-throughput operations.
* **Local Persistence & Caching**: Independently handles `Create` and `Get` operations for **L4 DEKs**, securely storing them in a local edge vault to ensure resilience and performance.
...
* **Eliminate Performance Bottlenecks** through a decentralized, edge-native cryptographic model designed for sub-millisecond global execution.